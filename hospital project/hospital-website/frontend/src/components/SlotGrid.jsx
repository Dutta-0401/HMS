...
const timeGroups = [
  { label: 'Morning', icon: Sun, filter: (h) => h >= 6 && h < 12, color: 'text-amber-500' },
  { label: 'Afternoon', icon: Sunset, filter: (h) => h >= 12 && h < 17, color: 'text-orange-500' },
  { label: 'Evening', icon: Moon, filter: (h) => h >= 17 || h < 6, color: 'text-violet-500' },
]
                  ...
                  <motion.button
                    ...
                    className={`relative p-3 rounded-xl text-sm font-medium transition-all duration-200 ${
                      isBooked
                        ...
                        : isSelected
                          ? 'bg-primary-600 text-white shadow-glow border border-primary-600'
